...
public class TestService {
  ...
  private void getRolesForIdentityCompare() throws IOException {
    ...
    while (scanner.hasNextLine()) {
      ...
      if (!compare(getRolesForIdentityResponse, request, new String[] {"ca.bc.gov.open.cso"})) {
        fileOutput.println(
            "\nINFO: getRolesForIdentity with AccountIdentifier"
                + inputs[0]
                + " Application "
  ...
  private void getRolesForIdentifierCompare() throws IOException {
    ...
    GetRolesForIdentifierResponse getRolesForIdentifierResponse = new GetRolesForIdentifierResponse();
    int diffCounter = 0;
    InputStream inputIds = getClass().getResourceAsStream("/getRolesForIdentifierInputs.csv");
    assert inputIds != null;
    Scanner scanner = new Scanner(inputIds);
    fileOutput = new PrintWriter(outputDir + "getRolesForIdentifier.txt", StandardCharsets.UTF_8);
    ...
    while (scanner.hasNextLine()) {
      ...
      if (!compare(getRolesForIdentifierResponse, request, new String[] {"ca.bc.gov.open.cso"})) {
        fileOutput.println(
                "\nINFO: getRolesForIdentifier with Application"
                        + inputs[0]
                        + " Domain "
  ...
  private void getRolesForApplicationCompare() throws IOException {
    ...
    GetRolesForApplicationResponse getRolesForApplicationResponse = new GetRolesForApplicationResponse();
    int diffCounter = 0;
    InputStream inputIds = getClass().getResourceAsStream("/getRolesForApplicationInputs.csv");
    assert inputIds != null;
    Scanner scanner = new Scanner(inputIds);
    fileOutput = new PrintWriter(outputDir + "getRolesForApplication.txt", StandardCharsets.UTF_8);
    ...
    while (scanner.hasNextLine()) {
      ...
      if (!compare(getRolesForApplicationResponse, request, new String[] {"ca.bc.gov.open.cso"})) {
        fileOutput.println(
                "\nINFO: getRolesForApplication with Application"
                        + inputs[0]
                        + " Domain "
